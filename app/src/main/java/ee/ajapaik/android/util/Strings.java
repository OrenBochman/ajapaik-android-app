...
public class Strings {
    public static String toBase16(byte b) {
        return new String( new char[] { (char)fromBase16((b >> 4) & 0x0F), (char)fromBase16((b) & 0x0F) });
    }
    ...
    public static String toLocalizedDate(Date date) {
    ...
    public static String toLocalizedDistance(Context context, float distanceInMeters) {
        return (distanceInMeters < 1000) ?
                String.format("%d %s", Math.round(distanceInMeters), context.getString(R.string.unit_m)) :
